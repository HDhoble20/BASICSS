...
void Bookrecord()
{
    ...
    {
        printf("\t\t\tRECOREDS\n\n");
         fgets("\t\t\t\t\t\t Student Name: %s",info.Book_name);
         fgets("\t\t\t\t\t\t Student Name: %s",info.Publication_name);
         fgets("\t\t\t\t\t\t roll num: %s",info.Author_name);
         gets("\t\t\t\t\t\t board: %d",info.Book_price);
         fgets("\t\t\t\t\t\t percentage: %s",info.section);
        exit(1);
    }
